(function () {
  ...
  function showTitle() {
    if (onlineActive) window.BoxOnline.leave();
    onlineActive = false;
    game = makeTitleGame();
  ...
  function fixedUpdate() {
    if (onlineActive || game.state !== "Playing" || game.paused) return;
  ...
  window.addEventListener("keydown", e => {
    ...
    if (onlineActive) {
      ...
      if (["KeyR", "Enter"].includes(e.code)) return;
    }
  ...
  document.addEventListener("visibilitychange", () => { if (document.hidden) { keys.clear(); mouse.left=mouse.right=false; resetTouchControls(); onlineShot=false; } });
  ...
  function bindStick(pad, knob, kind) {
    ...
    pad.addEventListener("pointerdown", e => {
      ...
      if (kind === "aim") {
        ...
        holdTimer = setTimeout(() => {
          ...
          const p1 = findPlayer(onlineActive ? onlinePlayer : 1);
          if (onlineActive) onlineShot = true;
          else if (game.state === "Playing" && !game.paused && p1 && p1.alive) tryFire(p1);
    ...
    window.addEventListener("blur", () => end({pointerId:activeId}));
  }
  ...
  window.BoxGame = {
    ...
    receive(model) {
      ...
      if(audio && game.online && model.events){
        ...
        if(model.events.mine>(game.events?.mine||0))audio.playMine();
    ...
    title:showTitle,
    input() {
      const tank=findPlayer(onlinePlayer);
      const angle=coarsePointer ? (touch.aiming?Math.atan2(touch.aimY,touch.aimX):(tank?.turretAngle||0)) : Math.atan2(mouse.y-(tank?.y||0),mouse.x-(tank?.x||0));
      const result={x:Math.abs(touch.moveX)>.05?touch.moveX:(down("KeyD")?1:0)-(down("KeyA")?1:0),
        y:Math.abs(touch.moveY)>.05?touch.moveY:(down("KeyS")?1:0)-(down("KeyW")?1:0),angle,
        fire:mouse.left||down("KeyF"),mine:mouse.right||down("Space")||touch.mine,shot:onlineShot};
      onlineShot=false;return result;
    }
  };
  renderer.render(toRenderModel());
  raf = requestAnimationFrame(loop);
