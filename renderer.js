(function () {
  ...
  class BoxTanksRenderer {
    ...
    render(model) {
      ...
      if (!model || model.state === "Title") {
        ...
        if ((model.missionIntroFrames || 0) > 0) {
          this.drawCenterBanner(`MISSION ${model.mission || 1}`, "white");
        }
        if (model.paused) {
        ...
        if (model.state === "GameOver" || model.state === "Complete") {
          ...
          ctx.fillStyle = "white";
          ctx.fillText("R = RESTART    ENTER = TITLE", W / 2, 405);
    ...
    drawHud(model) {
      ...
      ctx.textBaseline = "alphabetic";
      ctx.fillStyle = "gold";
      ctx.fillText(`MISSION ${model.mission || 1}`, 18, 31);
      ctx.fillStyle = "white";
      ctx.fillText(`DESTROYED ${model.totalDestroyed || 0}`, 170, 31);
      ...
      if (!model.twoPlayer) {
        ...
        ctx.fillStyle = "white";
        ctx.fillText("2P MAX 20", 635, 31);
      }

      ctx.font = "bold 12px 'Meiryo UI', sans-serif";
      ctx.fillText("ESC: PAUSE", 830, 31);
